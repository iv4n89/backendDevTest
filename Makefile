...
test-k6: ## Runs load tests with k6
	...
	@$(COMPOSE) exec $(BACKEND_SERVICE) sh -c 'timeout 60 sh -c "until wget -q --spider http://localhost:5000/actuator/health; do sleep 2; done"' || (echo "Backend failed to start" && exit 1)
